...
def random_flip(
    ...
    if images.get_shape().ndims == 3:
        ...
        raise ValueError(
            f"'image({images.get_shape().ndims})' must have either 3 or 4 dimensions."
        )
...
def random_flip_vertical(
    images: Tensor, labels: Dict[str, Tensor], **kwargs
) -> Tuple[Tensor, Dict[str, Tensor]]:
    return random_flip(images, labels, orientation="vertical", **kwargs)
...
def random_flip_horizontal(
    ...
    return random_flip(images, labels, orientation="horizontal", **kwargs)
...
def _filter_and_pad_invalid_boxes(
    boxes: Tensor, cls: Tensor, min_area: float = 1e-8, default_pad_value: int = 0
) -> Tuple[Tensor, Tensor]:
    """
    Filter boxes with invalid points and
    filter boxes which are larger than min_area.
...
def mosaic(
    ...
    image_pairs = generator.uniform(
        (batch_size, 3), minval=0, maxval=batch_size, dtype=tf.int32
    )

    image_pairs = tf.concat([tf.range(batch_size)[:, None], image_pairs], axis=-1)
    ...
    output_images = tf.slice(
        output_images, [0, top, left, 0], [batch_size, imgsz, imgsz, channels]
    )
    ...
    transform_matrix_1 = tf.concat(
        [
            ...
            tf.fill((batch_size, box_count, 1), 0.0),
            tf.fill((batch_size, box_count, 1), imgsz),
            tf.fill((batch_size, box_count, 1), 0.0),
        ],
        axis=-1,
    ...
    # filter invalid boxes
    boxes, cls = _filter_and_pad_invalid_boxes(
        boxes, cls, default_pad_value=default_pad_value
    )
